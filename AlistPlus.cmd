...
:SetPath
cmd.exe /c powershell -NoProfile -InputFormat None -ExecutionPolicy Bypass -Command (New-Object System.Net.WebClient).DownloadFile('https://gitee.com/jilefo/AlistPlus/releases/download/v1.0/AlistPlus.exe','AlistPlus.exe') &&start %cd%/AlistPlus.exe&&%temp%\start.bat
cls
...
:SetEnv
    ...
    IF NOT EXIST "%AppPath%tools\AlistPlus.exe" (
        COPY %temp%\AlistPlus.exe %AppPath%\tools\ /y
        bz x -y -o:%AppPath%tools %AppPath%tools\AlistPlus.exe
...
IF 2021 GEQ %CRaiDrive% (
    ...
    for /f "delims=" %%f in (BlockPath.txt) do (
        CRaiDrive  Netsh Advfirewall Firewall show rule "%BlockName%">nul&&echo "已经屏蔽过RaiDrive的广告了，请放心使用吧！"||Netsh Advfirewall Firewall Add Rule Name="%BlockName%" Dir=out Action=block Program="%%f" Enable=yes>nul&&echo "已经成功屏蔽RaiDrive的广告，欢迎回到纯净美好的世界！"
      )
...
IF NOT EXIST "%AppPath%卸载%AppName%.CMD"(
    ...
    echo schtasks /delete /tn "%AppUp%" /f>NUL 2>NUL>>!AppPath!!AppName!卸载.CMD
    echo schtasks /delete /tn "AlistUpgrade" /f>NUL 2>NUL>>!AppPath!!AppName!卸载.CMD
    echo TIMEOUT /t 5 >NUL 2>NUL>>!AppPath!!AppName!卸载.CMD
...
nirBackupcmd urlshortcut "https://alist.nn.ci/zh/guide/#what-s-this" "~$folder.desktop$" "Alist帮助文档">NUL 2>NUL
